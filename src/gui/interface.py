...
def show_inbreed(txt_field, idx):
    if len(idx) != 1:
        txt_field.configure(foreground="red", text=" Error: Select only one person to count its coefficient of inbreeding!")
        return
...
def show_relation(txt_field, idx):
    if len(idx) != 2:
        txt_field.configure(foreground="red", text="Error: Select two person to count their coefficient of relationship!")
        return
...
def create_hint_window():
    ...
    info2 = Message(hint_window, text=infoTxt2, anchor=W, width=600, font=("Helvetica", 16))
    
    close_button = Button(
        hint_window, text="Close window", width=20, command=lambda: hint_window.destroy()
    )
...
def create_window():
    ...
    sample_data_button = Button(
        window, text="Load sample data", width=20, command=lambda: open_sample_data(window, listbox, resultInbred)
    )
    inbred_button.pack()
